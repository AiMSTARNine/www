...
if (!empty($_POST['input_katmenu_validate'])) {
    ...
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Kategori Menu yang dimasukan Sudah Ada!")
        ...
        if ($query) {
            $message = '<script>alert("Data Kategori Menu Berhasil Ditambahkan!")
                    window.location="../katmenu"</script>';
        } else {
            $message = '<script>alert("Data Kategori Menu Gagal Ditambahkan!")
                    window.location="../katmenu"</script>';
        }
    }
}
